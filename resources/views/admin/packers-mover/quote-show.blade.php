...
@php
    $user = $quote->user;
    $quoteNumber = $quote->invoice_number ?: 'PMQT-' . str_pad((string) $quote->id, 6, '0', STR_PAD_LEFT);
    $userAddress = collect([
    ...
    $pickupAddress = $packersMoverAddress->pickup_address ?? $userAddress;
    $dropAddress = $packersMoverAddress->drop_address ?? null;
    $branchAddress = collect([$quote->packers_mover_name, $quote->packers_mover_address ?: $quote->packers_mover_city])->filter()->join(', ');
...
<div class="quote-page">
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
        <div>
            <h4 class="mb-0">Packers &amp; Movers Quote Details</h4>
            <small class="text-muted">{{ $quoteNumber }}</small>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('admin.packers_mover_leads') }}" class="btn btn-light">Back</a>
            <a href="{{ route('admin.packers_mover_lead.quote.download', $quote->packers_mover_lead_id) }}" class="btn btn-primary">Download</a>
        </div>
    </div>
    ...
    <div class="quote-sheet">
        <div class="quote-header d-flex justify-content-between align-items-start flex-wrap gap-3">
            <div>
                <h1 class="quote-title">Packers &amp; Movers Quote</h1>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    <div class="charge-card">
                        ...
                        <div class="info-value">{{ $dropAddress ?: '-' }}</div>
                        <div class="info-label mt-3">Distance</div>
                        <div class="info-value">{{ number_format((float) $quote->distance_km, 1) }} KM</div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="charge-card">
