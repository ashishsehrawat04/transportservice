...
<div class="quote-sheet">
    <div class="quote-header">
        <div style="display:flex;justify-content:space-between;align-items:center">
            <div>
                <div class="quote-title">Warehouse Storage Quote</div>
            ...
            <div style="text-align:right">
                ...
                <div style="font-weight:700">{{ $quote->invoice_number ?: ('WQT-' . str_pad((string) $quote->id, 6, '0', STR_PAD_LEFT)) }}</div>
    ...
    <div class="section">
        ...
        <div class="value">{{ $warehouseAddress?->pickup_address ?: (optional($quote->user)->address_line_1 ? collect([optional($quote->user)->address_line_1, optional($quote->user)->address_line_2, optional($quote->user)->city, optional($quote->user)->state, optional($quote->user)->country, optional($quote->user)->pincode])->filter()->join(', ') : '-') }}</div>
    </div>

    <div class="section">
        <div class="label">Warehouse</div>
        <div class="value">{{ collect([$quote->warehouse_name, $quote->warehouse_address ?: $quote->warehouse_city])->filter()->join(', ') ?: '-' }}</div>
    </div>
